...
## Пример подсветки синтаксиса для Python (PythonSyntax Plugin v 1.0)
    ...
    def replaceConsts(self, data: dict, constants: dict) -> str: """Устаревшая конструкция (Новые VtAPI.platform(), VtAPI.arch() и тд). Может заменить в тексте {platforn} и тд на систему"""
    ...
    def replacePaths(self, data: str) -> str: """Заменяет %переменные% их значениями в PATH"""
